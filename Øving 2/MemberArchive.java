...
public class MemberArchive {
    ...
    public int findPoints(int memberNo, String password) {
        return memberList.stream().filter(bonusMember -> (bonusMember.getMemberNo() == memberNo) && (bonusMember.okPassword(password))).findFirst().map(BonusMember::getPoints).orElse( 0);
    }
        ...
        while (i < memberList.size()) {
            if (memberList.get(i) instanceof BasicMember) {
                ...
                if (member1.getPoints() >= goldLimit) {
                    GoldMember newMember = new GoldMember(member1.getMemberNo(), member1.getPersonals(), member1.getEnrolledDate(), member1.getPoints());
                    memberList.set(i, newMember);
                    upgrade = true;
                } else {
                    if (member1.getPoints() >= silverLimit) {
                        SilverMember newMember = new SilverMember(member1.getMemberNo(), member1.getPersonals(), member1.getEnrolledDate(), member1.getPoints());
                        memberList.set(i, newMember);
                        upgrade = true;
                    }
                }
    ...
    int findAvailableNo() {
        ...
        int memberNo = ran.nextInt(1000000) + 1000000;
        boolean change = true;
        while (change) {
            ...
            if (!availableNumber) {
                memberNo = ran.nextInt(1000000) + 1000000;
            } else {
                change = false;
            }
        }
